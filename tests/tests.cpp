...
CTEST(GraphicTest, DrawSprites) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            ASSERT_TRUE(spritePosition.x == expectedX && spritePosition.y == expectedY);
        }
    }
}
...
CTEST(LogicTests, HandleKeyPress) {
    ...
    // Проверьте, что при нажатии клавиши валидного направления плитки перемещаются
    ...
    for (int i = 0; i < 4; ++i) {
        handleKeyPress(keys[i], grid, s, aValue, 128); // Передайте переменную aValue вместо временного значения
        ...
        ASSERT_TRUE(grid[originalX][originalY] == 0); // Плитка была удалена из старой позиции
